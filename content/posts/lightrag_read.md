...
不同的向量数据库的 `upsert` 流程有不同的实现，这里以默认 `NanoVectorDB` 实现讲解。
包括下面几个关键流程
...
然后对针对三部分数据再进行一轮的去重合并，通过循环遍历三个数据源（向量块、实体块、关系块）的相同索引位置，按照 **向量数据块 -> 实体数据块 -> 关系数据块 的固定优先级**顺序依次添加块到结果列表中。算法使用 seen_chunk_ids 集合进行严格去重，确保相同的 chunk_id 只被添加一次，即使它在多个数据源中都出现。
